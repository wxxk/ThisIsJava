...
public class FoodManager2 {
	...
	private static void insertFood() {
		...
		food.name = sc.next();
		System.out.print("수량: ");
		food.count = sc.nextInt();
		System.out.print("유통기한: ");
		food.expDate = sc.next();
		System.out.print("설명: ");
		food.etc = sc.next();
		foodList[count++] = food;
	} // end insertFood
	...
	private static void printFoodList() {
		...
		for (int i = 0; i < count; i++) {
			System.out.printf("%s\t%d\t%s\t%s", foodList[i].name, foodList[i].count, foodList[i].expDate, foodList[i].etc);
			System.out.println();
		}
		System.out.println("------------------------------");
	} // end printFoodList
	...
	private static void deleteFood() {
		...
		String food = sc.next();
		for (int i = 0; i < count; i++) {
			if (food.equals(foodList[i].name)) {
